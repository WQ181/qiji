<template>
	<view>
		...
		<view v-if="members == 0" style="margin-left: 30upx;">暂无用户咨询客服</view>
						...
						<view class="comment-top">
							...
							<view class="comment-content">
								<view v-if="item.user_content.contents[item.user_content.contents.length - 1].type == 'text'">
									...
									<view v-else>{{ item.user_content.contents[item.user_content.contents.length - 1].content }}</view>
								</view>
								<view v-else-if="item.user_content.contents[item.user_content.contents.length - 1].type == 'picture'">[图片]</view>
								<view v-else-if="item.user_content.contents[item.user_content.contents.length - 1].type == 'voice'">[语音]</view>
								<view v-else>[视频]</view>
...
<script>
	...
	export default {
		...
		async onLoad() {
			...
			all.globalData.kf_local_id = all.globalData.user_id + 'kf';
			try {
				console.log('kf_local_id', all.globalData.kf_local_id);
				const value = uni.getStorageSync(all.globalData.kf_local_id);
				if (value) {
					all.globalData.kf_msg = value;
					console.log('获取本地数据', value);
				}
			} catch (e) {
				// error
			}
			this.members = all.globalData.kf_msg;
		...
		onShow() {
			console.log("存储kf_id", all.globalData.kf_local_id)
			if (all.globalData.kf_chat_id) {
				all.globalData.kf_msg.some(item => {
					if (item.user_id == all.globalData.kf_chat_id) {

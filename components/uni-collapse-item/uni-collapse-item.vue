	 ...
	 :hover-class="disabled ? '' : 'uni-collapse-cell--hover'">
		<view class="uni-collapse-cell__title header" @click="onClick">
			...
			<view class="uni-collapse-cell__title-inner">
				<view class="uni-collapse-cell__title-text" style="font-size: 36upx;margin-left: 40upx;">{{ title }}</view>
			</view>
			<view :class="{ 'uni-active': isOpen, 'uni-collapse-cell--animation': showAnimation === true }" class="uni-collapse-cell__title-arrow">
				<uni-icon color="#bbb" size="30" type="arrowdown" />
			</view>
		</view>
		<view :class="{ 'uni-collapse-cell--animation': showAnimation === true }" :style="{  height: isOpen ? height :'0px' }"
		 class="uni-collapse-cell__content" style="border-radius: 20upx; background: linear-gradient(120deg, #5DC2FA, #4ED3FF);">
			<view :id="elId">
				<slot />
			</view>
		</view>
